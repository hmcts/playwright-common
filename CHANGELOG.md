...
## [1.1.6]
### Fixed
- Updated `IdamPage.login()` to support both single-form and split username/password IDAM login flows while preserving session-file storage.

## [1.1.5]
### Update
- update `IdamPage.login()` to support split IDAM sign-in flow: username + Continue, then password + Continue
